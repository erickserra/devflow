...
import { THEMES, displayThemes } from '@/constants/theme';
        ...
        >
          {displayThemes.map((theme) => (
            ...
            >
              <Image
                src={theme.icon}
                height={theme.value === THEMES.system ? 20 : 16}
                width={theme.value === THEMES.system ? 20 : 16}
                alt={theme.value}
                className={mode === theme.value ? 'active-theme' : ''}
              />
              <p
                className={`body-semibold text-light-500
                ${mode === theme.value ? 'text-primary-500' : 'text-dark100_light900'}`}
              >
                {theme.label}
              </p>
            </MenubarItem>
